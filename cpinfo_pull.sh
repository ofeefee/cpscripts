...
outfile="myfile.txt"
mgmtport="4434"
outputformat="1" #1=condensed#
# -----------------------------------

# Pull objects
echo "pulling objects"
objectjson=$(mgmt_cli -r true show gateways-and-servers --port $mgmtport --format json)

# Create target list
echo "creating target list"
mapfile -t gwarray < <(echo $objectjson | jq '.objects[] | select(.type=="simple-gateway") | .name' )
...
#    mgmt_cli -r true run-script script-name "run cpinfo" script "cpinfo -y all /" targets.1 "$name" --port $mgmtport  --format json 2> /dev/null | $CPDIR/jq/jq -r '(.tasks[0]["task-details"][0].responseMessage)' | base64 -di >> $outfile
fi
done
